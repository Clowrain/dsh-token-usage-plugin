...
declare global {
  interface Window {
    __ModuleLoader__: {
      load(opts: { id: string; factory: (require: (id: string) => unknown) => unknown }): void
    }
  }
}
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var React = require('react') as typeof import('react');
  ...
  .bmon-col-cache{display:none;}
      ...
      function OverviewSection({ stats, range }) {
        ...
        const tiles = [
          ...
          mk('t4', '缓存命中率',
            React.createElement('div', { className: 'bmon-u-tile-value' },
              hit === null ? '—' : React.createElement('span', { className: 'bmon-hit-badge bmon-hit-' + hitStatus }, Math.round(hit) + '%')),
            React.createElement('div', { className: 'bmon-u-tile-sub' },
      ...
      function UsageBars({ daily, range }) {
        ...
        children.push(React.createElement('div', { key: 'head', className: 'bmon-u-card-head' },
          ...
          React.createElement('span', { className: 'bmon-legend-box', style: { marginLeft: 'auto' } },
            React.createElement('span', { className: 'bmon-legend-item' + (vis.s1 ? '' : ' off'), onClick: () => toggleSeries('s1'), title: '点击开/关 新输入 系列' }, React.createElement('i', { style: { background: 'var(--bmon-c1)' } }), '新输入'),
            React.createElement('span', { className: 'bmon-legend-item' + (vis.s2 ? '' : ' off'), onClick: () => toggleSeries('s2'), title: '点击开/关 输出 系列' }, React.createElement('i', { style: { background: 'var(--bmon-c2)' } }), '输出'),
            React.createElement('span', { className: 'bmon-legend-item' + (vis.s3 ? '' : ' off'), onClick: () => toggleSeries('s3'), title: '点击开/关 缓存命中 系列' }, React.createElement('i', { style: { background: 'var(--bmon-c3)' } }), '缓存命中'),
            weekly ? React.createElement('span', { className: 'bmon-legend-item' }, '按周聚合') : null
        )))
      ...
      function UsageModal({ onClose }) {
        ...
        const syncPrices = () => {
          ...
          apiCall('sync-prices', {}).then((value) => {
            if (value && value.error) showToast('同步单价失败，请稍后重试')
            else showToast('同步单价成功')
        ...
        const head = React.createElement('div', { key: 'head', className: 'bmon-overlay-head' },
          ...
          React.createElement('div', { className: 'bmon-head-actions' },
            ...
            React.createElement('button', { type: 'button', className: 'bmon-iconbtn bmon-tip', 'data-tip': '从远程同步最新模型单价', title: '从远程同步最新模型单价', 'aria-label': '同步单价', disabled: syncing, onClick: syncPrices },
              syncing ? React.createElement('span', { className: 'bmon-spin' }, cloudIcon) : cloudIcon),
            React.createElement('button', { type: 'button', className: 'bmon-iconbtn', title: '刷新统计数据', onClick: () => { apiCall('get-usage', { range: range, heatYear: heatYear }).then((v) => { if (v && typeof v === 'object') pair[1](v) }).catch(() => {}) } }, '↻'),
            React.createElement('button', { type: 'button', className: 'bmon-closebtn', title: '关闭弹窗', onClick: onClose }, '✕ 关闭'),
          ),
        )
        const bodyChildren = []
        if (!snapshot) {
          ...
          bodyChildren.push(React.createElement(Section, {
            ...
            React.createElement('div', { key: 'table', className: 'bmon-table-panel' },
              React.createElement('div', { className: 'bmon-table-toolbar' },
                ...
                React.createElement('div', { className: 'bmon-dropdown' },
                  React.createElement('button', { type: 'button', className: 'bmon-btn bmon-col-toggle' + (colsOpen ? ' on' : ''), onClick: () => setColsOpen((v) => !v) }, '列设置 ▾'),
                  colsOpen ? React.createElement('div', { className: 'bmon-dropdown-menu' },
                    React.createElement('label', { className: 'bmon-dropdown-item' },
                      React.createElement('input', { type: 'checkbox', checked: showCache, onChange: (e) => setShowCache(e.target.checked) }),
                      '缓存命中'),
                  ) : null,
                ),
                React.createElement('button', { type: 'button', className: 'bmon-btn', title: '导出当前模型用量明细 CSV 文件', onClick: exportCsv }, '导出 CSV'),
        ...
        return React.createElement('div', { className: 'bmon-overlay', onClick: onClose },
          React.createElement('div', { className: 'bmon-overlay-card', onClick: (e) => e.stopPropagation() },
            toast ? React.createElement('div', { key: 'toast', className: 'bmon-toast' }, toast) : null,
            head,
      ...
      slots.inject('conversation.input.right', () => (ctx.get('slots') as {
        register(entry: { name: string; id: string; order: number; label: string }, component: () => unknown): unknown
      }).register(
