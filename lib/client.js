...
.bmon-u-record-scroll{max-width:100%;height:300px;overflow:auto;contain:strict;content-visibility:auto;contain-intrinsic-size:300px 680px;overscroll-behavior:contain;}
.bmon-u-record-scroll .bmon-table{min-width:680px;table-layout:fixed;}
.bmon-u-record-scroll .bmon-table th,.bmon-u-record-scroll .bmon-table td{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.bmon-u-record-scroll .bmon-table th:nth-child(1),.bmon-u-record-scroll .bmon-table td:nth-child(1){width:180px;}
.bmon-u-record-scroll .bmon-table th:nth-child(2),.bmon-u-record-scroll .bmon-table td:nth-child(2){width:80px;}
.bmon-u-record-scroll .bmon-table th:nth-child(3),.bmon-u-record-scroll .bmon-table td:nth-child(3){width:160px;}
.bmon-u-record-scroll .bmon-table th:nth-child(4),.bmon-u-record-scroll .bmon-table td:nth-child(4),.bmon-u-record-scroll .bmon-table th:nth-child(5),.bmon-u-record-scroll .bmon-table td:nth-child(5),.bmon-u-record-scroll .bmon-table th:nth-child(6),.bmon-u-record-scroll .bmon-table td:nth-child(6){width:86px;}
...
.bmon-u-heat-scroll{position:relative;max-width:100%;overflow-x:auto;overflow-y:visible;padding:18px 2px 4px;}
.bmon-u-heat-months{position:absolute;top:0;left:30px;height:14px;min-width:100%;}
.bmon-u-heat-month{position:absolute;top:0;font-size:10px;color:var(--dsw-alias-label-secondary);white-space:nowrap;}
.bmon-u-heat-layout{display:flex;gap:6px;align-items:flex-start;width:max-content;min-width:100%;}
.bmon-u-heat-weekdays{display:grid;grid-template-rows:repeat(7,10px);gap:3px;flex:none;font-size:9px;color:var(--dsw-alias-label-secondary);}
.bmon-u-heat-weekdays span{height:10px;line-height:10px;}
.bmon-u-heat-grid{display:grid;grid-template-rows:repeat(7,10px);grid-auto-flow:column;grid-auto-columns:10px;gap:3px;flex:none;}
.bmon-u-heat-cell{display:block;}
...
.bmon-u-num{text-align:right;}`)
      ...
      function UsageHeatmap({ daily, range }) {
        if (!daily || !daily.length) return React.createElement('div', { className: 'bmon-u-card' },
          React.createElement('div', { className: 'bmon-u-card-head' },
            React.createElement('h3', { style: { margin: 0, fontSize: 13, fontWeight: 600 } }, '用量日历'),
            React.createElement('span', { className: 'bmon-hint' }, '暂无日用量数据'),
          ),
        )
        const byDate = new Map(daily.map((d) => [d.date, d]))
        const latest = parseDate(daily[daily.length - 1].date)
        const first = range === 'all'
          ? new Date(latest.getFullYear(), latest.getMonth(), latest.getDate() - 364)
          : parseDate(daily[0].date)
        ...
        for (const cursor = new Date(start); cursor <= end; cursor.setDate(cursor.getDate() + 1)) {
          ...
          days.push(byDate.get(key) || { date: key, requests: 0, total: 0, prompt: 0, completion: 0, cache_read: 0 })
        }
        ...
        const cells = days.map((day) => {
          ...
          return React.createElement('i', { key: day.date, className: 'bmon-u-heat-cell', 'data-l': String(level), 'data-tip': tip })
        })
        const monthLabels = []
        let previous = ''
        days.forEach((day, index) => {
          ...
          if (month !== previous) {
            monthLabels.push(React.createElement('span', { key: month, className: 'bmon-u-heat-month', style: { left: (Math.floor(index / 7) * 13) + 'px' } }, month.slice(5) + '月'))
            previous = month
          }
        })
        ...
        return React.createElement('div', { className: 'bmon-u-card' },
          React.createElement('div', { className: 'bmon-u-card-head' },
            React.createElement('h3', { style: { margin: 0, fontSize: 13, fontWeight: 600 } }, '用量日历'),
            React.createElement('span', { className: 'bmon-hint' }, range === 'all' ? '最近 365 天 · GitHub 贡献图风格' : 'GitHub 贡献图风格'),
          ),
          React.createElement('div', { className: 'bmon-u-heat-scroll' },
            React.createElement('div', { className: 'bmon-u-heat-months' }, ...monthLabels),
            React.createElement('div', { className: 'bmon-u-heat-layout' },
              React.createElement('div', { className: 'bmon-u-heat-weekdays' }, ...weekdays),
              React.createElement('div', { className: 'bmon-u-heat-grid' }, ...cells),
            ),
          ),
      ...
      function UsagePage() {
        ...
        children.push(React.createElement(UsageHeatmap, { key: 'heat', daily: stats.daily, range: range }))
