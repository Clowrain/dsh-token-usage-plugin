...
.bmon-proxy{display:flex;flex-direction:column;gap:4px;background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l1);border-radius:8px;padding:8px 10px;}
.bmon-proxy-select,.bmon-proxy-input{background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-label-primary);border-radius:6px;padding:3px 8px;font-size:12px;font-family:inherit;}
.bmon-proxy-select:focus,.bmon-proxy-input:focus{outline:2px solid color-mix(in srgb,var(--dsw-alias-brand-primary) 35%,transparent);outline-offset:1px;border-color:var(--dsw-alias-brand-primary);}
body[data-ds-dark-theme] .bmon-proxy-select,body[data-ds-dark-theme] .bmon-proxy-input{color-scheme:dark;}
body:not([data-ds-dark-theme]) .bmon-proxy-select,body:not([data-ds-dark-theme]) .bmon-proxy-input{color-scheme:light;}
.bmon-proxy-input{flex:1 1 180px;min-width:0;}
...
/* —— 用量日历（GitHub 贡献图风格 · 自然年 1/1~12/31，可按年份筛选）—— */
...
.bmon-gh-hint{font-size:11px;color:var(--dsw-alias-label-secondary);flex:1 1 0;min-width:0;}
.bmon-gh-select{flex:none;margin-left:auto;background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-label-primary);border-radius:6px;padding:3px 8px;font-size:12px;font-family:inherit;cursor:pointer;}
.bmon-gh-select:focus{outline:2px solid color-mix(in srgb,var(--dsw-alias-brand-primary) 35%,transparent);outline-offset:1px;border-color:var(--dsw-alias-brand-primary);}
body[data-ds-dark-theme] .bmon-gh-select{color-scheme:dark;}
body:not([data-ds-dark-theme]) .bmon-gh-select{color-scheme:light;}
...
.bmon-u-num{text-align:right;}`)
      ...
      function useUsageState(range, heatYear) {
        ...
        React.useEffect(() => {
          let alive = true
          let timer = null
          lastVersion.current = -1 // 切换 range/年份后强制渲染一次（版本是全局的）
          ...
          const tick = () => {
            ...
            apiCall('get-usage', { range: range, heatYear: heatYear }).then((value) => {
              ...
              if (value && typeof value === 'object') {
                // 数据版本或所选年份变化才 setState，避免整棵面板重渲染
      ...
      function UsageTiles({ stats, range }) {
        ...
        const tiles = [
          React.createElement('div', { key: 't0', className: 'bmon-u-tile' },
            React.createElement('div', { className: 'bmon-u-tile-label' }, '预估费用'),
            React.createElement('div', { className: 'bmon-u-tile-value' }, fmtMoney(totals.cost)),
            React.createElement('div', { className: 'bmon-u-tile-sub' }, totals.cost > 0 ? '按模型单价换算' : '暂无价格，可同步'),
          ),
      ...
      function UsageHeatmap({ heat, heatYears, year, onYearChange }) {
        const data = heat || []
        const tipState = React.useState(null)
        const tip = tipState[0]
        const setTip = tipState[1]
        const years = Array.isArray(heatYears) && heatYears.length ? heatYears : (year ? [year] : [])
        const selectedYear = year || (years.length ? years[years.length - 1] : new Date().getFullYear())
        const empty = React.createElement('div', { className: 'bmon-gh-card' },
        ...
        // 自然年 1/1~12/31：从 1/1 对齐到周一往前补格，渲染到 12/31
        ...
        const weekEls = weeks.map((col, w) => {
          const cells = col.map((day, r) => {
            // 超出所选年份的格子（首周补的上年、尾周补的下年）渲染为透明占位
        ...
        // 月份标签：按周序号用百分比定位，随网格弹性拉伸自适应；跳过跨年补格列
        ...
        const yearSelect = React.createElement('select', {
          key: 'year', className: 'bmon-gh-select', value: String(selectedYear),
          onChange: (e) => { if (onYearChange) onYearChange(Number(e.target.value)) },
        ...
        return React.createElement('div', { className: 'bmon-gh-card' },
          ...
          React.createElement('div', { className: 'bmon-gh-head' },
            ...
            React.createElement('span', { className: 'bmon-gh-hint' },
              '共 ' + (totalTokens > 0 ? fmtTokens(totalTokens) : '0') + ' tokens · ' +
              totalRequests.toLocaleString() + ' 次调用 · ' + activeDays + ' 天活跃'),
            yearSelect,
          ),
      ...
      function UsageModels({ stats }) {
        ...
        const rows = models.map((model, index) => {
          ...
          const cells = [
            ...
            React.createElement('td', { key: 'cost', className: 'bmon-u-num' }, fmtMoney(model.cost)),
          ]
        ...
        const table = React.createElement('div', { className: 'bmon-u-table-scroll' },
          React.createElement('table', { className: 'bmon-table' },
            React.createElement('thead', null, React.createElement('tr', { key: 'h' },
              ...
              React.createElement('th', { key: 'cost', className: 'bmon-u-num' }, '费用'),
            )),
      ...
      function UsagePage() {
        ...
        const [proxyMode, setProxyMode] = React.useState('auto')
        const [proxySys, setProxySys] = React.useState('')
        ...
        const loadProxyCfg = () => {
          apiCall('get-proxy-config', {}).then((v) => {
            if (!v || typeof v !== 'object') return
            setProxyMode(v.mode || 'auto')
            setProxySys(v.systemProxy || '')
          }).catch(() => {})
        }
        if (!proxyOpen && proxyMode === 'auto') loadProxyCfg()
        const saveProxyCfg = () => {
          setProxyMsg('')
          apiCall('set-proxy-config', { mode: proxyMode }).then((v) => {
            setProxyMsg(v && v.mode === 'off' ? '代理：已关闭（直连）' : '代理：自动（跟随系统）')
          }).catch((e) => setProxyMsg('保存失败：' + String(e && e.message || e)))
        }
        const syncPrices = () => {
          if (syncing) return
          setSyncing(true)
        ...
        const proxyPanel = proxyOpen ? React.createElement('div', { key: 'proxy', className: 'bmon-proxy' },
          React.createElement('div', { className: 'bmon-row' },
            React.createElement('label', { className: 'bmon-hint' }, '网络代理'),
            React.createElement('select', { className: 'bmon-proxy-select', value: proxyMode, onChange: (e) => setProxyMode(e.target.value) },
              React.createElement('option', { value: 'auto' }, '自动（跟随系统）'),
              React.createElement('option', { value: 'off' }, '关（直连）'),
            ),
            React.createElement('button', { type: 'button', className: 'bmon-btn', onClick: saveProxyCfg }, '保存'),
            proxyMode === 'auto' && proxySys
              ? React.createElement('span', { className: 'bmon-hint' }, '检测到系统代理：' + proxySys)
              : null,
          ),
          proxyMsg ? React.createElement('div', { key: 'pm', className: 'bmon-hint' }, proxyMsg) : null,
        ) : null
        children.push(React.createElement('div', { key: 'toolbar', className: 'bmon-u-toolbar' },
          ...
          React.createElement('button', { type: 'button', className: 'bmon-btn', onClick: () => { setProxyOpen(!proxyOpen); if (!proxyOpen) loadProxyCfg() } }, '代理'),
          React.createElement('button', { type: 'button', className: 'bmon-btn', disabled: syncing, onClick: syncPrices }, syncing ? '同步中…' : '同步价格'),
          React.createElement('button', { type: 'button', className: 'bmon-btn', onClick: () => { apiCall('get-usage', { range: range, heatYear: heatYear }).then((value) => { if (value && typeof value === 'object') pair[1](value) }).catch(() => {}) } }, '↻'),
          syncMsg ? React.createElement('span', { key: 'syncmsg', className: 'bmon-hint' }, syncMsg) : null,
        ))
        if (proxyPanel) children.push(proxyPanel)
        if (!snapshot) {
