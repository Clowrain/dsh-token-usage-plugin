...
.bmon-gh-months{position:relative;height:14px;margin-left:24px;margin-bottom:2px;}
.bmon-gh-month{position:absolute;top:0;font-size:10px;color:var(--dsw-alias-label-secondary);white-space:nowrap;}
.bmon-gh-body{display:flex;gap:2px;width:max-content;}
...
.bmon-gh-days span{height:10px;line-height:10px;}
.bmon-gh-week{display:flex;flex-direction:column;gap:2px;flex:none;}
.bmon-gh-cell{width:10px;height:10px;border-radius:2px;background:var(--bmon-h0);}
...
.bmon-u-num{text-align:right;}`)
      ...
      function UsageHeatmap({ heat }) {
        ...
        // 月份标签：列首日期跨月时打点（近似 GitHub）。10px 格 + 2px 间距 = 12px/列
        const monthLabels = []
        let prevMonth = ''
        weeks.forEach((col, w) => {
          ...
          if (month !== prevMonth) {
            monthLabels.push(React.createElement('span', { key: month, className: 'bmon-gh-month', style: { left: (w * 12) + 'px' } }, month.slice(5) + '月'))
            prevMonth = month
          }
        })
