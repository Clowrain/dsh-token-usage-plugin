...
.bmon-gh-months{position:relative;height:14px;margin-left:30px;margin-bottom:2px;}
.bmon-gh-month{position:absolute;top:0;font-size:10px;color:var(--dsw-alias-label-secondary);white-space:nowrap;}
.bmon-gh-body{display:flex;gap:3px;width:max-content;}
.bmon-gh-days{display:flex;flex-direction:column;gap:3px;flex:none;width:14px;font-size:9px;color:var(--dsw-alias-label-secondary);}
.bmon-gh-days span{height:11px;line-height:11px;}
.bmon-gh-week{display:flex;flex-direction:column;gap:3px;flex:none;}
.bmon-gh-cell{width:11px;height:11px;border-radius:2.5px;background:var(--bmon-h0);}
...
.bmon-gh-cell[data-tip]:hover::after{content:attr(data-tip);position:absolute;left:50%;bottom:calc(100% + 7px);transform:translateX(-50%);z-index:30;min-width:max-content;max-width:240px;padding:5px 8px;border:1px solid var(--dsw-alias-border-l2);border-radius:5px;background:var(--dsw-alias-bg-layer-3,var(--dsw-alias-bg-layer-1));color:var(--dsw-alias-label-primary);box-shadow:0 4px 14px rgba(0,0,0,.26);font-size:11px;font-weight:400;line-height:1.4;white-space:pre-line;pointer-events:none;}
...
.bmon-u-num{text-align:right;}`)
      ...
      function UsageHeatmap({ heat }) {
        ...
        // 固定展示最近 53 周（GitHub 贡献图风格），与范围 tab 无关
        const today = new Date()
        today.setHours(0, 0, 0, 0)
        const start = new Date(today)
        start.setDate(start.getDate() - 371)
        // 周一起始（GitHub 中文版惯例）：getDay() 周一=1，回退到最近的周一
        ...
        for (let w = 0; w < 53; w += 1) {
          ...
          for (let r = 0; r < 7; r += 1) {
            ...
            col.push(byDate.get(key) || { date: key, requests: 0, total: 0 })
          }
          weeks.push(col)
        }
        ...
        const weekEls = weeks.map((col, w) => {
          const cells = col.map((day, r) => {
            ...
            return React.createElement('i', { key: day.date, className: 'bmon-gh-cell', 'data-l': String(lv), 'data-tip': tip })
          })
          return React.createElement('div', { key: 'w' + w, className: 'bmon-gh-week' }, ...cells)
        })
        // 月份标签：列首日期跨月时打点（近似 GitHub）
        const monthLabels = []
        let prevMonth = ''
        weeks.forEach((col, w) => {
          ...
          if (month !== prevMonth) {
            monthLabels.push(React.createElement('span', { key: month, className: 'bmon-gh-month', style: { left: (w * 14) + 'px' } }, month.slice(5) + '月'))
            prevMonth = month
          }
        })
