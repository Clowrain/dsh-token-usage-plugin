...
//
// 迁移 TS 说明：保持 ModuleLoader bundle 形态（无 JSX / 无 CSS Modules），
// 仅用 TS 类型标注复用 DSH 类型；构建由 tsc 编 CommonJS + 薄包装脚本生成最终 bundle。
...
.bmon-overlay-card{background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l2);border-radius:14px;width:min(1080px,100%);max-height:92vh;display:flex;flex-direction:column;box-shadow:0 14px 44px rgba(0,0,0,.38);container-type:inline-size;}
...
.bmon-head-updated{font-size:11px;color:var(--dsw-alias-label-secondary);opacity:.6;white-space:nowrap;}
...
.bmon-u-seg button.on{background:var(--dsw-alias-brand-primary);color:var(--dsw-alias-label-on-brand,var(--dsw-alias-bg-layer-1));font-weight:600;box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--dsw-alias-brand-primary) 60%,transparent);}
body[data-ds-dark-theme] .bmon-u-seg button.on{box-shadow:inset 0 0 0 1px rgba(255,255,255,.25);}
...
.bmon-u-tile{background:var(--dsw-alias-bg-layer-2,var(--dsw-alias-bg-layer-1));border:1px solid var(--dsw-alias-border-l1);border-radius:10px;padding:8px 12px;display:flex;flex-direction:column;gap:3px;min-width:0;}
.bmon-u-tile-label{font-size:11px;color:var(--dsw-alias-label-secondary);display:flex;align-items:center;gap:6px;}
.bmon-u-tile-value{font-size:24px;font-weight:600;line-height:1.2;color:var(--dsw-alias-label-primary);}
.bmon-u-tile-value small{font-size:11px;font-weight:400;color:var(--dsw-alias-label-secondary);margin-left:4px;}
.bmon-u-tile-sub{font-size:10px;color:var(--dsw-alias-label-secondary);opacity:.7;display:flex;flex-direction:column;gap:2px;}
.bmon-hit-badge{display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border-radius:999px;font-size:15px;font-weight:700;line-height:1.3;border:1px solid transparent;}
.bmon-hit-ok{background:color-mix(in srgb,var(--dsw-alias-state-success-primary) 20%,transparent);color:var(--dsw-alias-state-success-primary);border-color:color-mix(in srgb,var(--dsw-alias-state-success-primary) 45%,transparent);}
.bmon-hit-warn{background:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 22%,transparent);color:var(--dsw-alias-state-warn-primary);border-color:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 45%,transparent);}
.bmon-hit-err{background:color-mix(in srgb,var(--dsw-alias-state-error-primary) 20%,transparent);color:var(--dsw-alias-state-error-primary);border-color:color-mix(in srgb,var(--dsw-alias-state-error-primary) 45%,transparent);}
...
/* —— 图表区 —— */
.bmon-gh-card{background:var(--dsw-alias-bg-layer-2,var(--dsw-alias-bg-layer-1));border:1px solid var(--dsw-alias-border-l1);border-radius:10px;padding:8px 10px;overflow:hidden;min-width:0;display:flex;flex-direction:column;}
.bmon-gh-head{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:4px;}
...
.bmon-gh-scroll{overflow-x:auto;overflow-y:hidden;padding:12px 0 2px;position:relative;flex:1 1 0;min-height:0;min-height:140px;}
...
.bmon-gh-body{display:flex;gap:2px;box-sizing:border-box;width:100%;align-items:stretch;padding-right:4px;margin-top:8px;height:100%;}
...
.bmon-gh-week{display:flex;flex-direction:column;gap:2px;flex:1 1 0;min-width:0;align-items:stretch;height:100%;}
.bmon-gh-cell{flex:1 1 0;width:100%;min-width:10px;min-height:10px;border-radius:3px;background:var(--bmon-h0);}
...
.bmon-u-bars-wrap{position:relative;padding-top:18px;}
.bmon-u-bars{position:relative;height:168px;display:flex;align-items:flex-end;gap:3px;border-bottom:1px solid var(--dsw-alias-border-l1);padding-left:32px;}
.bmon-u-bar-slot{flex:1;min-width:0;display:flex;flex-direction:column;justify-content:flex-end;align-items:center;height:168px;}
...
.bmon-legend-box{display:inline-flex;gap:10px;align-items:center;font-size:11px;color:var(--dsw-alias-label-secondary);white-space:nowrap;}
.bmon-legend-item{display:inline-flex;gap:4px;align-items:center;cursor:pointer;user-select:none;}
.bmon-legend-item:hover{color:var(--dsw-alias-label-primary);}
.bmon-legend-item.off{opacity:.35;text-decoration:line-through;}
.bmon-legend-item i{display:inline-block;width:9px;height:9px;border-radius:2px;flex:none;}
/* —— 模型明细 —— */
...
.bmon-toast{position:absolute;top:56px;left:50%;transform:translateX(-50%);z-index:2147483005;background:var(--dsw-alias-bg-layer-3,var(--dsw-alias-bg-layer-1));border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-primary);border-radius:8px;padding:6px 14px;font-size:12px;box-shadow:0 6px 18px rgba(0,0,0,.3);pointer-events:none;animation:bmon-toast-in .18s ease;}
@keyframes bmon-toast-in{from{opacity:0;transform:translateX(-50%) translateY(-4px)}to{opacity:1;transform:translateX(-50%) translateY(0)}}
.bmon-spin{display:inline-flex;animation:bmon-spin .9s linear infinite;}
@keyframes bmon-spin{to{transform:rotate(360deg)}}
.bmon-dropdown{position:relative;display:inline-flex;}
.bmon-dropdown-menu{position:absolute;top:calc(100% + 4px);left:0;z-index:2147483004;background:var(--dsw-alias-bg-layer-3,var(--dsw-alias-bg-layer-1));border:1px solid var(--dsw-alias-border-l2);border-radius:8px;box-shadow:0 6px 18px rgba(0,0,0,.28);padding:4px;min-width:120px;}
.bmon-dropdown-item{display:flex;gap:6px;align-items:center;font-size:12px;color:var(--dsw-alias-label-primary);padding:5px 8px;border-radius:6px;cursor:pointer;white-space:nowrap;}
.bmon-dropdown-item:hover{background:var(--dsw-alias-bg-layer-2,var(--dsw-alias-bg-layer-1));}
.bmon-dropdown-item input{margin:0;accent-color:var(--dsw-alias-brand-primary);}
/* —— 响应式（容器查询：以弹窗卡片宽度为准）—— */
...
@container (max-width:599px){
  ...
  .bmon-col-cache{display:none;}
            ...
            function Section({ title, extra, collapsed, onToggle, children }) {
                return React.createElement('section', { className: 'bmon-sec' + (collapsed ? ' bmon-sec-collapsed' : '') }, React.createElement('div', { className: 'bmon-sec-head', onClick: onToggle }, React.createElement('span', { className: 'bmon-sec-arrow' }, collapsed ? '▸' : '▾'), React.createElement('span', { className: 'bmon-sec-title' }, title), extra ? React.createElement('span', { className: 'bmon-sec-extra' }, extra) : null), collapsed ? null : React.createElement('div', { className: 'bmon-sec-body' }, children));
            }
            // ===== 概览统计 =====
            function OverviewSection({ stats, range }) {
                ...
                const delta = (current, previous) => {
                    ...
                    return React.createElement('span', { key: 'd', className: 'bmon-tip', 'data-tip': '对比上一周期' }, value >= 0 ? '▲' : '▼', ' ', Math.abs(value).toFixed(0) + '%');
                };
                ...
                const priceRows = pricePopup && pricePopup.prices
                    ? pricePopup.prices.map((p) => React.createElement('tr', { key: p.model }, React.createElement('td', null, p.model), React.createElement('td', { className: 'bmon-u-num' }, '$' + Number(p.prompt).toFixed(3)), React.createElement('td', { className: 'bmon-u-num' }, '$' + Number(p.completion).toFixed(3)), React.createElement('td', { className: 'bmon-u-num' }, '$' + Number(p.cacheRead).toFixed(4))))
                    : null;
                const mk = (key, label, valueEl, footerEl, tip) => React.createElement('div', { key: key, className: 'bmon-u-tile bmon-tip', 'data-tip': tip || '' }, React.createElement('div', { className: 'bmon-u-tile-label' }, label), valueEl, footerEl);
                const tiles = [
                    mk('t0', '预估费用', React.createElement('div', { className: 'bmon-u-tile-value' }, fmtMoney(totals.cost)), React.createElement('div', { className: 'bmon-u-tile-sub' }, React.createElement('span', null, totals.cost > 0 ? '按模型单价换算' : '暂无价格，可同步'), React.createElement('span', {
                        ...
                        onMouseMove: (e) => { if (pricePopup)
                            setPricePopup({ x: e.clientX + 12, y: e.clientY - 8, prices: pricePopup.prices }); },
                        onMouseLeave: () => setPricePopup(null),
                    }, '查看单价')), '当前范围预估费用 ' + fmtMoney(totals.cost) + ' USD'),
                    mk('t1', '总消耗', React.createElement('div', { className: 'bmon-u-tile-value' }, fmtTokens(totals.total), React.createElement('small', null, 'tokens'), delta(totals.total, prev && prev.total)), React.createElement('div', { className: 'bmon-u-tile-sub' }, React.createElement('div', null, '输入 ' + fmtTokens(totals.prompt)), React.createElement('div', null, '输出 ' + fmtTokens(totals.completion))), '总消耗 ' + fullNum(totals.total) + ' tokens（输入 ' + fullNum(totals.prompt) + ' / 输出 ' + fullNum(totals.completion) + '）'),
                    mk('t2', '轮次 / 步数', React.createElement('div', { className: 'bmon-u-tile-value' }, String(totals.turns), React.createElement('small', null, '轮')), React.createElement('div', { className: 'bmon-u-tile-sub' }, React.createElement('div', null, '步数 ' + String(totals.steps)), React.createElement('div', null, '工具调用 ' + totals.tools + ' 次')), '轮次 ' + fullNum(totals.turns) + ' · 步数 ' + fullNum(totals.steps) + ' · 工具调用 ' + fullNum(totals.tools) + ' 次'),
                    mk('t3', '请求数', React.createElement('div', { className: 'bmon-u-tile-value' }, Number(totals.requests || 0).toLocaleString(), delta(totals.requests, prev && prev.requests)), React.createElement('div', { className: 'bmon-u-tile-sub' }, React.createElement('div', null, '全部模型调用'), dailyAvg ? React.createElement('div', null, '日均 ' + dailyAvg) : null), '请求数 ' + fullNum(totals.requests) + (dailyAvg ? ' · 日均 ' + dailyAvg : '')),
                    mk('t4', '缓存命中率', React.createElement('div', { className: 'bmon-u-tile-value' }, hit === null ? '—' : React.createElement('span', { className: 'bmon-hit-badge bmon-hit-' + hitStatus }, Math.round(hit) + '%')), React.createElement('div', { className: 'bmon-u-tile-sub' }, React.createElement('div', null, '命中 ' + fmtTokens(totals.cache_read)), React.createElement('div', null, '未命中 ' + fmtTokens(fresh))), '缓存命中 ' + fullNum(totals.cache_read) + ' / 未命中 ' + fullNum(fresh) + ' tokens'),
                ];
                const pricePopover = pricePopup ? React.createElement('div', { className: 'bmon-price-pop' }, React.createElement('div', { className: 'bmon-price-pop-title' }, '模型单价（USD / 1M tokens）'), priceRows && priceRows.length ? React.createElement('table', { className: 'bmon-table' }, React.createElement('thead', null, React.createElement('tr', null, React.createElement('th', null, '模型'), React.createElement('th', { className: 'bmon-u-num' }, '输入'), React.createElement('th', { className: 'bmon-u-num' }, '输出'), React.createElement('th', { className: 'bmon-u-num' }, '缓存命中'))), React.createElement('tbody', null, ...priceRows)) : React.createElement('div', { className: 'bmon-hint' }, '暂无缓存单价，请先同步价格')) : null;
                return React.createElement('div', { className: 'bmon-ov-grid' }, pricePopover ? React.createElement('div', { key: 'popwrap', className: 'bmon-price-pop-wrap', style: { left: pricePopup.x, top: pricePopup.y } }, pricePopover) : null, ...tiles);
            }
            // ===== 用量日历（GitHub 贡献图，自然年）=====
            function UsageHeatmap({ heat }) {
                ...
                if (!data.length)
                    return React.createElement('div', { className: 'bmon-gh-card' }, React.createElement('div', { className: 'bmon-gh-head' }, React.createElement('h3', { className: 'bmon-gh-title' }, '用量日历'), React.createElement('span', { className: 'bmon-gh-hint' }, selectedYear + ' 年暂无日用量数据')));
                ...
                const weekEls = weeks.map((col, w) => {
                    const cells = col.map((day, r) => {
                        if (!day || !inYear(day.date))
                            return React.createElement('i', { key: 'e' + r, className: 'bmon-gh-cell bmon-gh-cell-blank' });
                ...
                const weekdays = ['一', '二', '三', '四', '五', '六', '日'].map((label, r) => React.createElement('span', { key: r }, r % 2 === 0 ? label : ''));
                const legend = [0, 1, 2, 3, 4].map((lv) => React.createElement('i', { key: lv, 'data-l': String(lv) }));
                return React.createElement('div', { className: 'bmon-gh-card' }, tip ? React.createElement('div', { className: 'bmon-gh-tooltip', style: { left: tip.x, top: tip.y } }, tip.text) : null, React.createElement('div', { className: 'bmon-gh-head' }, React.createElement('h3', { className: 'bmon-gh-title' }, '用量日历'), React.createElement('span', { className: 'bmon-gh-hint' }, selectedYear + ' 年 · 共 ' + (totalTokens > 0 ? fmtTokens(totalTokens) : '0') + ' tokens · ' + totalRequests.toLocaleString() + ' 次调用')), React.createElement('div', { className: 'bmon-gh-scroll' }, React.createElement('div', { className: 'bmon-gh-months' }, ...monthLabels), React.createElement('div', { className: 'bmon-gh-body' }, React.createElement('div', { className: 'bmon-gh-days' }, ...weekdays), ...weekEls)), React.createElement('div', { className: 'bmon-gh-footer' }, React.createElement('span', null, '少'), ...legend, React.createElement('span', null, '多')));
            }
            // ===== 消耗趋势（三段堆叠柱状图，图例可点击开关系列）=====
            function UsageBars({ daily, range }) {
                const visible = React.useState({ s1: true, s2: true, s3: true });
                const vis = visible[0];
                const setVis = visible[1];
                const toggleSeries = (key) => setVis((v) => ({ ...v, [key]: !v[key] }));
                let slice = daily || [];
                let weekly = false;
                if (range === '1d')
                    slice = slice.slice(-1);
                    ...
                    for (let week = 0; week < weeks; week += 1) {
                        ...
                        for (const day of chunk) {
                            m.requests += day.requests;
                            m.prompt += day.prompt;
                            m.completion += day.completion;
                            m.cache_read += day.cache_read;
                            m.total += day.total;
                        }
                        merged.push(m);
                    }
                ...
                const HEIGHT = 168;
                const visibleTotal = (d) => (vis.s1 ? Math.max(0, d.prompt - d.cache_read) : 0) + (vis.s2 ? d.completion : 0) + (vis.s3 ? d.cache_read : 0);
                const max = Math.max(...slice.map((d) => visibleTotal(d)), 0);
                const children = [];
                if (!max) {
                ...
                slice.forEach((day, index) => {
                    ...
                    const segments = [];
                    const series = [['s1', fresh, 'bmon-u-s1'], ['s2', day.completion, 'bmon-u-s2'], ['s3', day.cache_read, 'bmon-u-s3']];
                    for (const pair of series) {
                        if (!vis[pair[0]])
                            continue;
                        segments.push(React.createElement('i', { key: pair[2], className: pair[2], style: { height: Math.max(pair[1] > 0 ? 1 : 0, (pair[1] / max) * HEIGHT) + 'px' } }));
                    }
                    const fullNum = (v) => Number(v || 0).toLocaleString();
                    const tip = day.date + (weekly ? ' 起当周' : '') + '\n新输入 ' + fullNum(fresh) + '\n输出 ' + fullNum(day.completion) + '\n缓存命中 ' + fullNum(day.cache_read) + '\n请求 ' + day.requests + ' · 合计 ' + fullNum(day.total);
                    bars.push(React.createElement('div', { key: day.date, className: 'bmon-u-bar-slot bmon-tip', 'data-tip': tip }, React.createElement('div', { className: 'bmon-u-bar-col' }, ...segments)));
                    let labelText = '';
                    if (weekly)
                        labelText = index % 4 ? '' : day.date.slice(5);
                    else if (slice.length > 16)
                        labelText = index % 5 ? '' : day.date.slice(5);
                    else if (slice.length === 1)
                        labelText = day.date.slice(5);
                    else
                        labelText = day.date.slice(8);
                    xlabels.push(React.createElement('span', { key: day.date }, labelText));
                });
                children.push(React.createElement('div', { key: 'head', className: 'bmon-u-card-head' }, React.createElement('h3', { style: { margin: 0, fontSize: 12, fontWeight: 600 } }, '消耗趋势'), React.createElement('span', { className: 'bmon-legend-box', style: { marginLeft: 'auto' } }, React.createElement('span', { className: 'bmon-legend-item' + (vis.s1 ? '' : ' off'), onClick: () => toggleSeries('s1'), title: '点击开/关 新输入 系列' }, React.createElement('i', { style: { background: 'var(--bmon-c1)' } }), '新输入'), React.createElement('span', { className: 'bmon-legend-item' + (vis.s2 ? '' : ' off'), onClick: () => toggleSeries('s2'), title: '点击开/关 输出 系列' }, React.createElement('i', { style: { background: 'var(--bmon-c2)' } }), '输出'), React.createElement('span', { className: 'bmon-legend-item' + (vis.s3 ? '' : ' off'), onClick: () => toggleSeries('s3'), title: '点击开/关 缓存命中 系列' }, React.createElement('i', { style: { background: 'var(--bmon-c3)' } }), '缓存命中'), weekly ? React.createElement('span', { className: 'bmon-legend-item' }, '按周聚合') : null)));
                children.push(React.createElement('div', { key: 'chart', className: 'bmon-u-bars-wrap' }, React.createElement('div', { className: 'bmon-u-bars' }, ...grid, ...bars), React.createElement('div', { className: 'bmon-u-x' }, ...xlabels)));
                return React.createElement('div', { className: 'bmon-gh-card' }, ...children);
            }
            // ===== 模型饼图（Top4 + 其他，可点击过滤）=====
            function PieDonut({ models, filter, onFilter, totalRequests }) {
                ...
                if (rest.length)
                    items.push({ key: '__other__', label: '其他（' + rest.length + '）', value: rest.reduce((s, m) => s + m.total, 0), color: gray });
                let acc = 0;
                const sectors = items.map((item) => {
                    const pctVal = total > 0 ? (item.value / total) * 100 : 0;
                    const reqCount = item.key === '__other__'
                        ? rest.reduce((s, m) => s + (m.requests || 0), 0)
                        : (top.find((m) => m.model === item.key) || { requests: 0 }).requests;
                    const sector = React.createElement('circle', {
                        ...
                        title: item.label + ' · 占比 ' + (total > 0 ? (item.value / total * 100).toFixed(1) : '0.0') + '% · 请求 ' + reqCount + ' 次\n点击筛选表格',
                ...
                }, React.createElement('span', { className: 'bmon-pie-legend-dot', style: { background: item.color } }), React.createElement('span', { className: 'bmon-pie-legend-name' }, item.label), React.createElement('span', { className: 'bmon-pie-legend-pct' }, (total > 0 ? (item.value / total * 100).toFixed(1) : '0.0') + '%')));
                return React.createElement('div', { className: 'bmon-pie-panel' }, React.createElement('div', {
                    className: 'bmon-pie-wrap',
                    title: '点击扇区筛选表格数据',
                    onClick: (e) => { if (e.target === e.currentTarget)
                        onFilter(''); },
                }, React.createElement('svg', { viewBox: '0 0 42 42' }, ...sectors), React.createElement('div', { className: 'bmon-pie-center' }, React.createElement('b', null, Number(totalRequests || 0).toLocaleString()), React.createElement('small', null, '次请求'))), React.createElement('div', { className: 'bmon-pie-legend' }, ...legend));
            }
            ...
            function ModelTable({ models, topNames, filter, search, onSearch, sortKey, sortDir, onSort, showCache, page, onPage }) {
                ...
                let rows = (models || []).filter((m) => {
                    if (filter === '__other__') {
                        if (topNames.has(m.model))
                            return false;
                    }
                    else if (filter && m.model !== filter)
                        return false;
                    if (query && m.model.toLowerCase().indexOf(query) === -1)
                        return false;
                    return true;
                });
                ...
                const trs = pageRows.map((m) => React.createElement('tr', { key: m.model }, React.createElement('td', null, React.createElement('span', { style: { display: 'inline-block', width: 8, height: 8, borderRadius: 2, background: colorOf(m), marginRight: 6, verticalAlign: 'middle' } }), m.model), React.createElement('td', { key: 'r', className: 'bmon-u-num' }, Number(m.requests || 0).toLocaleString()), React.createElement('td', { key: 'c', className: 'bmon-u-num bmon-money' }, fmtMoney(m.cost)), React.createElement('td', { key: 'i', className: 'bmon-u-num' }, fmtTokens(m.input)), React.createElement('td', { key: 'o', className: 'bmon-u-num' }, fmtTokens(m.output)), showCache ? React.createElement('td', { key: 'h', className: 'bmon-u-num bmon-col-cache' }, m.cacheRead ? fmtTokens(m.cacheRead) : '—') : null));
                const table = React.createElement('div', { className: 'bmon-table-scrollbox' }, React.createElement('table', { className: 'bmon-table' }, React.createElement('thead', null, React.createElement('tr', null, React.createElement('th', { key: 'm' }, '模型'), sortTh('requests', '请求数', true), sortTh('cost', '费用', true), sortTh('input', '输入 tokens', true), sortTh('output', '输出 tokens', true), showCache ? sortTh('cacheRead', '缓存命中', true) : null)), React.createElement('tbody', null, trs.length ? trs : React.createElement('tr', null, React.createElement('td', { colSpan: 6, className: 'bmon-u-empty' }, '无匹配模型')))));
                const pager = React.createElement('div', { className: 'bmon-pager' }, React.createElement('span', null, '共 ' + rows.length + ' 个模型 · 第 ' + (safePage + 1) + '/' + pageCount + ' 页'), React.createElement('button', { type: 'button', className: 'bmon-btn bmon-page', disabled: safePage <= 0, onClick: () => onPage(safePage - 1) }, '‹'), React.createElement('button', { type: 'button', className: 'bmon-btn bmon-page', disabled: safePage >= pageCount - 1, onClick: () => onPage(safePage + 1) }, '›'));
            ...
            function UsageModal({ onClose }) {
                ...
                const syncPrices = () => {
                    ...
                    apiCall('sync-prices', {}).then((value) => {
                        if (value && value.error)
                            showToast('同步单价失败，请稍后重试');
                        else
                            showToast('同步单价成功');
                        apiCall('get-usage', { range: range, heatYear: heatYear }).then((v) => { if (v && typeof v === 'object')
                            pair[1](v); }).catch(() => { });
                    }).catch(() => showToast('同步单价失败，请稍后重试'))
                ...
                const exportCsv = () => {
                    ...
                    let rows = stats.models.filter((m) => {
                        if (pieFilter === '__other__') {
                            if (tops.has(m.model))
                                return false;
                        }
                        else if (pieFilter && m.model !== pieFilter)
                            return false;
                        if (query && m.model.toLowerCase().indexOf(query) === -1)
                            return false;
                        return true;
                ...
                const cloudIcon = React.createElement(Icon, { paths: '<path d="M17.5 19H9a7 7 0 1 1 6.71-9h1.79a4.5 4.5 0 1 1 0 9Z"/><path d="M20 9V5"/><path d="M18 7h4"/>', size: 16 });
                const head = React.createElement('div', { key: 'head', className: 'bmon-overlay-head' }, React.createElement('span', { className: 'bmon-overlay-title' }, '用量统计'), React.createElement('div', { className: 'bmon-head-filters' }, React.createElement('div', { className: 'bmon-u-seg' }, ...ranges.map((r) => React.createElement('button', { key: r[0], type: 'button', className: r[0] === range ? 'on' : '', onClick: () => { setRange(r[0]); setPage(0); } }, r[1]))), React.createElement('select', {
                    className: 'bmon-head-select', value: String(selYear),
                    onChange: (e) => setHeatYear(Number(e.target.value)),
                    title: '日历年份',
                }, years.map((yy) => React.createElement('option', { key: yy, value: String(yy) }, yy + ' 年')))), React.createElement('div', { className: 'bmon-head-actions' }, React.createElement('span', { className: 'bmon-head-updated' }, snapshot ? '更新于 ' + new Date(snapshot.updatedAt).toLocaleTimeString() : '载入中…'), React.createElement('button', { type: 'button', className: 'bmon-iconbtn bmon-tip', 'data-tip': '从远程同步最新模型单价', title: '从远程同步最新模型单价', 'aria-label': '同步单价', disabled: syncing, onClick: syncPrices }, syncing ? React.createElement('span', { className: 'bmon-spin' }, cloudIcon) : cloudIcon), React.createElement('button', { type: 'button', className: 'bmon-iconbtn', title: '刷新统计数据', onClick: () => { apiCall('get-usage', { range: range, heatYear: heatYear }).then((v) => { if (v && typeof v === 'object')
                        pair[1](v); }).catch(() => { }); } }, '↻'), React.createElement('button', { type: 'button', className: 'bmon-closebtn', title: '关闭弹窗', onClick: onClose }, '✕ 关闭')));
                const bodyChildren = [];
                if (!snapshot) {
                    ...
                    // 阅读链路：总费用/总量 → 日期分布（用量日历）→ 时序拆解（消耗趋势）→ 模型明细
                    // 用量日历：独占整行
                    bodyChildren.push(React.createElement(Section, {
                        key: 'cal', title: '用量日历', collapsed: collapsed.charts, onToggle: () => toggle('charts'),
                        extra: '共 ' + fmtTokens(s.totals.total) + ' tokens · ' + Number(s.totals.requests || 0).toLocaleString() + ' 次调用',
                    }, React.createElement(UsageHeatmap, { key: 'heat', heat: s.heat })));
                    // 消耗趋势：独占整行
                    bodyChildren.push(React.createElement(Section, {
                        key: 'bars', title: '消耗趋势', collapsed: collapsed.charts, onToggle: () => toggle('charts'),
                        extra: '新输入 / 输出 / 缓存命中',
                    ...
                    }, React.createElement('div', { className: 'bmon-models-grid' }, React.createElement(PieDonut, {
                        key: 'pie', models: s.models, filter: pieFilter,
                        onFilter: (f) => { setPieFilter(f); setPage(0); },
                        totalRequests: s.totals.requests,
                    }), React.createElement('div', { key: 'table', className: 'bmon-table-panel' }, React.createElement('div', { className: 'bmon-table-toolbar' }, React.createElement('input', {
                        className: 'bmon-search-input', placeholder: '筛选模型', value: search,
                        onChange: (e) => { setSearch(e.target.value); setPage(0); },
                    }), React.createElement('div', { className: 'bmon-dropdown' }, React.createElement('button', { type: 'button', className: 'bmon-btn bmon-col-toggle' + (colsOpen ? ' on' : ''), onClick: () => setColsOpen((v) => !v) }, '列设置 ▾'), colsOpen ? React.createElement('div', { className: 'bmon-dropdown-menu' }, React.createElement('label', { className: 'bmon-dropdown-item' }, React.createElement('input', { type: 'checkbox', checked: showCache, onChange: (e) => setShowCache(e.target.checked) }), '缓存命中')) : null), React.createElement('button', { type: 'button', className: 'bmon-btn', title: '导出当前模型用量明细 CSV 文件', onClick: exportCsv }, '导出 CSV'), pieFilter ? React.createElement('button', { type: 'button', className: 'bmon-btn', onClick: () => { setPieFilter(''); setPage(0); } }, '清除过滤') : null), mt.table, mt.pager))));
                }
                return React.createElement('div', { className: 'bmon-overlay', onClick: onClose }, React.createElement('div', { className: 'bmon-overlay-card', onClick: (e) => e.stopPropagation() }, toast ? React.createElement('div', { key: 'toast', className: 'bmon-toast' }, toast) : null, head, React.createElement('div', { className: 'bmon-overlay-body' }, ...bodyChildren)));
            }
            function IconBarButton({ paths, label }) {
                ...
                const children = [
                    React.createElement('button', { key: 'b', type: 'button', className: 'bmon-ibar' + (open ? ' bmon-ibar-on' : ''), onClick: () => setOpen(!open), 'aria-label': label, title: label }, React.createElement(Icon, { paths: paths, size: 16 })),
                ];
                if (open) {
            ...
            // DSH 运行时注入的完整 slots（含 dsh-client-ui-conversation 声明的
            // conversation.input.right 等槽位）。此处按需收窄，避免依赖对话 UI 包的
            // 类型合并；实际调用由 DSH 提供同名 API。
            const slots = ctx.get('slots');
            if (slots === undefined)
                return;
            slots.inject('conversation.input.right', () => ctx.get('slots').register({ name: 'conversation.input.right', id: 'tusage-usage', order: 100, label: '用量统计' }, () => React.createElement(IconBarButton, { paths: '<path d="M6 20V10"/><path d="M12 20V4"/><path d="M18 20v-6"/>', label: '用量统计' })));
        }
        exports.apply = apply;
