...
export function apply(ctx) {
    ...
    function flushPending() {
        if (!db || !stmts || (!pendingWatermarks.size && !pendingDayStats.size))
            return;
        db.exec('BEGIN');
        try {
            for (const [id, row] of pendingWatermarks)
                stmts.setWatermark.run(id, row.seq, row.at);
            for (const [day, stats] of pendingDayStats)
                stmts.bumpDayStat.run(day, stats.turns, stats.steps, stats.tools);
            db.exec('COMMIT');
        ...
        try {
            if (existsSync(shardDir) && !stmts.getMeta.get('migratedShards')) {
                ...
                for (const name of readdirSync(shardDir)) {
                    ...
                    let shard = null;
                    try {
                        shard = JSON.parse(readFileSync(path.join(shardDir, name), 'utf8'));
                    }
                    catch (e) {
                        continue;
                    }
                    if (!shard || typeof shard !== 'object')
                        continue;
                    const day = m[1];
                    const t = (shard.total && typeof shard.total === 'object' ? shard.total : {});
                    ...
                    if (t.requests) {
                        ...
                        stmts.insertCall.run('legacy', Date.parse(day + 'T12:00:00'), day, 'legacy-aggregate', t.input || 0, t.output || 0, t.cacheRead || 0);
                    }
                    days += 1;
                }
                // 水位线迁移
                ...
                if (days)
                    console.log('[用量统计] 已迁移 ' + days + ' 天分片缓存 → SQLite（聚合以 legacy-aggregate 记录导入）');
            }
            // 删除旧目录与 v1 单文件（幂等）
            try {
    ...
    function deferredMaintenance() {
        ...
        try {
            console.log('[用量统计] 当前累计 ' + num(stmts.callCount.get()?.n) + ' 条调用记录');
        }
        catch (e) { /* ignore */ }
        bumpUsage();
    }
    ...
    function matchModelPrice(model, catalogModels) {
        ...
        const candidates = [];
        const add = (model, price, score) => { if (price)
            candidates.push({ model, price, score }); };
    ...
    async function syncPricesFromCatalog() {
        if (!stmts)
            return { ok: false, error: '存储未就绪', source: 'models.dev' };
        ...
        try {
            ...
            catalog = json && typeof json === 'object' ? json : null;
        }
        catch (e) {
            return { ok: false, error: '价格源不可达：' + String(e && e.message || e).slice(0, 120), source: source };
        }
    ...
    ctx.on('session/event', (session, event) => {
        if (!session || !event || typeof event.time !== 'number')
            return;
    ...
    function pruneSessions() {
        if (!db || !stmts)
            return;
        try {
            stmts.pruneSessions.run(Date.now() - 90 * 86400000);
        }
        catch (e) { /* ignore */ }
    }
    async function scanHistory() {
        if (!sessionQuery || !stmts)
            return;
        try {
            ...
            for (const record of sessions) {
                ...
                if (headerAt && headerAt <= watermarkAt(id)) {
                    skipped += 1;
                    continue;
                }
                try {
            ...
            if (skipped)
                console.log('[用量统计] ' + skipped + ' 个会话已追平，跳过解压');
        }
        catch (e) {
    ...
    function computeUsage(range, heatYear) {
        ...
        if (range === '1d') {
            since = dayOffset(0);
            prevSince = { from: dayOffset(1), to: dayOffset(1) };
        }
        else if (range === '7d') {
            since = dayOffset(6);
            prevSince = { from: dayOffset(13), to: dayOffset(7) };
        }
        else if (range === '30d') {
            since = dayOffset(29);
            prevSince = { from: dayOffset(59), to: dayOffset(30) };
        }
        ...
        const callsByDay = new Map(stmts.dailyRange.all(since, sinceTo).map((row) => [str(row.day), toDailyRow(row)]));
        const statsByDay = new Map(stmts.statsRange.all(since, sinceTo).map((row) => [str(row.day), toStatsRow(row)]));
        ...
        const heatCalls = new Map(stmts.dailyRange.all(yearFrom, yearTo).map((row) => [str(row.day), toDailyRow(row)]));
        const heatStats = new Map(stmts.statsRange.all(yearFrom, yearTo).map((row) => [str(row.day), toStatsRow(row)]));
        ...
        const heatYears = [];
        for (let yy = Math.min(minYear, nowYear); yy <= nowYear; yy += 1)
            heatYears.push(yy);
