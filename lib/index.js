...
export function apply(ctx) {
  ...
  function computeUsage(range) {
    ...
    // GitHub 贡献图：始终返回最近 365 天的完整日序列（与范围 tab 无关），
    // 供用量日历固定渲染一整年，不做任何范围联动。
    const heatSince = dayOffset(364)
    const heatCalls = new Map(stmts.callsDaily.all(heatSince).map((row) => [row.day, row]))
    const heatStats = new Map(stmts.statsDaily.all(heatSince).map((row) => [row.day, row]))
