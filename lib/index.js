...
export function apply(ctx) {
  ...
  function openDb() {
    ...
    stmts = {
      ...
      dailyModelTokens: db.prepare('SELECT day, model, IFNULL(SUM(input),0) AS input, IFNULL(SUM(output),0) AS output, IFNULL(SUM(cache_read),0) AS cache_read FROM calls WHERE day >= ? AND day <= ? GROUP BY day, model'),
      minCallDay: db.prepare('SELECT MIN(day) AS d FROM calls'),
      callCount: db.prepare('SELECT COUNT(*) AS n FROM calls'),
      listModels: db.prepare('SELECT DISTINCT model FROM calls'),
      allPrices: db.prepare('SELECT model, prompt_price, completion_price, cache_read_price, source, updated_at FROM model_prices'),
      upsertPrice: db.prepare('INSERT INTO model_prices(model, prompt_price, completion_price, cache_read_price, source, updated_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(model) DO UPDATE SET prompt_price = excluded.prompt_price, completion_price = excluded.completion_price, cache_read_price = excluded.cache_read_price, source = excluded.source, updated_at = excluded.updated_at'),
      countPrices: db.prepare('SELECT COUNT(*) AS n FROM model_prices'),
  ...
  // ===== 代理设置（自动跟随系统 / 关闭直连）=====
  // 利用 Node 22 全局 fetch 的 NODE_USE_ENV_PROXY：每次请求前按模式设置环境变量，
  // 请求后恢复。auto=跟随系统代理（env 或 macOS scutil）；off=直连。
  function getProxyConfig() {
    const mode = (() => { try { const m = stmts.getMeta.get('proxyMode'); return m ? m.value : '' } catch (e) { return '' } })()
    const addr = (() => { try { const m = stmts.getMeta.get('proxyAddr'); return m ? m.value : '' } catch (e) { return '' } })()
    return { mode: mode === 'off' ? 'off' : 'auto', addr: addr }
  }
  ...
  function detectSystemProxy() {
    ...
    try {
      const out = execFileSync('/usr/sbin/scutil', ['--proxy'], { encoding: 'utf8', timeout: 3000 })
      const host = (out.match(/HTTPSProxy\s*:\s*([^\n]+)/) || [])[1]
      const port = (out.match(/HTTPSPort\s*:\s*([^\n]+)/) || [])[1]
      const enabled = (out.match(/HTTPSEnable\s*:\s*([^\n]+)/) || [])[1]
      if (enabled && String(enabled).trim() === '1' && host) {
        const h = String(host).trim()
        const p = String(port || '').trim()
        return { proxy: 'http://' + h + (p ? ':' + p : ''), source: 'system' }
      }
    } catch (e) { /* 读取失败则回退 */ }
    return { proxy: '', source: '' }
  }
  ...
  async function fetchWithProxy(url) {
    ...
    const savedEnv = {}
    const savedFlag = process.env.NODE_USE_ENV_PROXY
    let envProxy = ''
    let envSource = ''
    if (cfg.mode === 'auto') {
      const sys = detectSystemProxy()
      envProxy = sys.proxy
      envSource = sys.source
    }
    // 备份并清理旧的代理 env
    for (const key of PROXY_ENV_KEYS) {
      if (process.env[key] !== undefined) { savedEnv[key] = process.env[key]; delete process.env[key] }
    }
    process.env.NODE_USE_ENV_PROXY = envProxy ? '1' : '0'
    if (envProxy) process.env.HTTPS_PROXY = envProxy
    try {
      ...
      return { ok: true, data: data, proxyUsed: envProxy ? envSource : '' }
    } catch (e) {
      return { ok: false, error: String(e && e.message || e).slice(0, 160), proxyUsed: envProxy ? envSource : '' }
    } finally {
      // 恢复环境变量
      for (const key of PROXY_ENV_KEYS) {
      ...
      if (savedFlag === undefined) delete process.env.NODE_USE_ENV_PROXY
      else process.env.NODE_USE_ENV_PROXY = savedFlag
    }
  }

  function computeCost(input, output, cacheRead, price) {
  ...
  function loadPrices() {
    ...
    for (const row of stmts.allPrices.all()) {
      map.set(row.model, [row.prompt_price, row.completion_price, row.cache_read_price])
    }
    return map
  }
  ...
  function matchModelPrice(model, catalogModels) {
    const suffix = stripModelPrefix(model)
    const candidates = []
    const add = (entries, score) => { for (const e of entries) candidates.push([e, score]) }
    // catalogModels: Map<normalizedKey, priceEntry>
    const exact = (name) => catalogModels.get(normalizeModelKey(name))
    if (suffix !== model) {
      if (exact(suffix)) add([{ model: suffix, price: exact(suffix) }], 100)
      if (exact(model)) add([{ model: model, price: exact(model) }], 90)
    } else {
      if (exact(model)) add([{ model: model, price: exact(model) }], 100)
    }
  ...
  async function syncPricesFromCatalog() {
    ...
    const fetched = await fetchWithProxy(url)
    if (!fetched.ok) {
      return { ok: false, error: '价格源不可达：' + (fetched.error || ''), source: source, proxyUsed: fetched.proxyUsed }
    }
    const catalog = fetched.data && typeof fetched.data === 'object' ? fetched.data : null
    if (!catalog) return { ok: false, error: '价格源返回数据格式异常', source: source, proxyUsed: fetched.proxyUsed }
    ...
    const index = new Map()
    const familyRank = (p) => (p === 'deepseek' ? 0 : p.indexOf('deepseek') >= 0 ? 1 : 100)
    ...
    // 同步我们 calls 中出现过的模型
    const models = stmts.listModels.all().map((r) => r.model)
    let synced = 0
    let unmatched = []
    const now = Date.now()
  ...
  ;(() => {
    try {
      if (stmts.countPrices.get().n === 0) {
        const now = Date.now()
        for (const [model, prices] of Object.entries(BUILTIN_PRICES)) {
          stmts.upsertPrice.run(model, prices[0], prices[1], prices[2], 'builtin', now)
        }
      }
  ...
  function computeUsage(range, heatYear) {
    ...
    let priceRev = ''
    try { const m = stmts.getMeta.get('priceSyncedAt'); priceRev = m ? String(m.value) : '' } catch (e) { /* ignore */ }
    ...
    for (const row of stmts.dailyModelTokens.all(since, sinceTo)) {
      ...
      if (!price) continue
      const cost = computeCost(row.input, row.output, row.cache_read, price)
      if (cost == null) continue
      dayCost.set(row.day, (dayCost.get(row.day) || 0) + cost)
      modelCost.set(row.model, (modelCost.get(row.model) || 0) + cost)
    }
    ...
    const heatCost = new Map()
    for (const row of stmts.dailyModelTokens.all(yearFrom, yearTo)) {
      const price = prices.get(row.model)
      if (!price) continue
      const cost = computeCost(row.input, row.output, row.cache_read, price)
      if (cost != null) heatCost.set(row.day, (heatCost.get(row.day) || 0) + cost)
    }
    ...
    const models = stmts.modelsRange.all(since, sinceTo).map((row) => ({
      ...
      cost: modelCost.get(row.model) || 0,
    }))
  ...
  // 手动触发价格同步（从 models.dev 拉取并缓存到 model_prices，按代理设置走网络）
  ...
  // 读取/保存代理配置（auto=自动跟随系统代理；off=关闭直连）
  registerRoute('get-proxy-config', async () => {
    const cfg = getProxyConfig()
    const sys = cfg.mode === 'auto' ? detectSystemProxy() : { proxy: '', source: '' }
    return { mode: cfg.mode, systemProxy: sys.proxy, systemSource: sys.source }
  })
  registerRoute('set-proxy-config', async (args) => {
    const mode = args && args.mode === 'off' ? 'off' : 'auto'
    stmts.setMeta.run('proxyMode', mode)
    stmts.setMeta.run('proxyAddr', '')
    return { mode: mode }
  })
}
