...
export function apply(ctx) {
  ...
  function openDb() {
    ...
    stmts = {
      ...
      callsTotalsRange: db.prepare('SELECT COUNT(*) AS requests, IFNULL(SUM(input),0) AS input, IFNULL(SUM(output),0) AS output, IFNULL(SUM(cache_read),0) AS cache_read FROM calls WHERE day >= ? AND day < ?'),
      ...
      callsDailyRange: db.prepare('SELECT day, COUNT(*) AS requests, IFNULL(SUM(input),0) AS input, IFNULL(SUM(output),0) AS output, IFNULL(SUM(cache_read),0) AS cache_read FROM calls WHERE day >= ? AND day <= ? GROUP BY day ORDER BY day'),
      statsDailyRange: db.prepare('SELECT day, turns, steps, tools FROM day_stats WHERE day >= ? AND day <= ?'),
  ...
  function advanceWatermark(id, seq) {
    ...
    if (!liveSeqAt.has(id)) {
      const row = db.prepare('SELECT at FROM sessions WHERE session_id = ?').get(id)
      liveSeqAt.set(id, row ? row.at : 0)
    }
  ...
  function computeUsage(range, heatYear) {
    ...
    let prevTotals = null
    if (prevSince) {
      const prev = stmts.callsTotalsRange.get(prevSince.from, prevSince.to)
    ...
    // heatYears = 首个有数据年份 → 当前年份，供前端年份筛选。
    ...
    const heatFrom = y + '-01-01'
    const heatTo = y + '-12-31'
    const heatCalls = new Map(stmts.callsDailyRange.all(heatFrom, heatTo).map((row) => [row.day, row]))
    const heatStats = new Map(stmts.statsDailyRange.all(heatFrom, heatTo).map((row) => [row.day, row]))
